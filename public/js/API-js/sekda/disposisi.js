document.addEventListener('DOMContentLoaded', async () => {
    ...
    // View Riwayat Disposisi
    try {
        const response = await fetch('/viewDisposisiSekre', {
            method: 'GET',
            headers: {
        ...
        if (data.success && data.data.length > 0) {
            ...
            data.data.forEach((surat, index) => {
                ...
                const disposisiStatuses = surat.dataDisposisi.map(disposisi => disposisi.status);

                if (disposisiStatuses.includes('Ditolak')) {
                    statusText = '<span class="text-danger">Surat Permohonan Magang Ditolak</span>';
                } else if (disposisiStatuses.includes('Disetujui')) {
                    statusText = '<span class="text-success">Surat Permohonan Magang Diterima</span>';
                } else{
                    statusText = '<span class="text-warning">Surat Permohonan Magang Sedang Diperiksa</span>';
                }
                ...
                detailButton.addEventListener('click', async () => {
                    try {
                        const detailResponse = await fetch(`/detailDisposisiSekre/${surat.id_surat_masuk}`, {
                            method: 'GET',
                            headers: {
                                'Content-type': 'application/json'
                            },
